...
export default function MilitaryOps() {
  ...
  const executeSearch = useCallback(async (query) => {
    ...
    setSearching(true);
    try {
      // Search live military feed
      const liveRes = await fetch(`${API_BASE}/api/search/military?q=${encodeURIComponent(query)}`);
      const liveData = liveRes.ok ? await liveRes.json() : { data: [] };

      // Search historical squawk events (military only)
      const histRes = await fetch(`${API_BASE}/api/search/history?q=${encodeURIComponent(query)}&category=military`);
  ...
  if (isSearchActive) {
    ...
    headingText = <>&gt;&gt;&gt; HISTORICAL_SQUAWK_LOG // MILITARY (LAST 30 DAYS)</>;
  } else {
    headingText = <>&gt;&gt;&gt; MILITARY_EMERGENCY_SQUAWKS (7500/7600/7700)</>;
  }
          ...
          {!loading && !error && !searching && displayEntries.length === 0 && (
            <div className="text-body-md glow-text" style={{ textAlign: 'center', padding: '32px 0', opacity: 0.6 }}>
              {isSearchActive
                ...
                : <>&gt;&gt;&gt; NO ACTIVE OR HISTORICAL MILITARY EMERGENCY SQUAWKS DETECTED</>}
            </div>
          )}
